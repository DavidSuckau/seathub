...
export function CreatePartForm({ projectId }: { projectId: string }) {
  ...
  return (
    <Panel title="Bauteil anlegen" className="mb-6">
      ...
      {mode === "pair" ? (
        <p className="mb-4 rounded-lg bg-[var(--accent-soft)] px-3 py-2 text-sm text-[var(--accent)]">
          Eine Seite wird entwickelt, die andere bekommt eine eigene Teilenummer als Spiegel – für
          Doku und Kundenzeichnung, ohne doppelte Entwicklung.
        </p>
      ) : null}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Modul-Art">
          <select
            ...
            onChange={(e) => setForm({ ...form, moduleKind: e.target.value as ModuleKind })}
          >
        ...
        <Field label="Bezeichnung">
          <input
            ...
            placeholder="z. B. Sportsitz Alcantara"
          />
        </Field>
        {mode === "pair" ? (
          <>
            <Field label="Welche Seite wird entwickelt?">
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, developSide: e.target.value as "links" | "rechts" })
                }
              >
                <option value="links">Links (Fahrer) – Rechts = Spiegel</option>
            ...
            <Field label="Teilenummer Links">
              <input
                ...
                onChange={(e) => setForm({ ...form, partNumberLeft: e.target.value })}
              />
            </Field>
            <Field label="Teilenummer Rechts">
              <input
                ...
                onChange={(e) => setForm({ ...form, partNumberRight: e.target.value })}
              />
            </Field>
          </>
        ) : (
          <>
            ...
            <div className="sm:col-span-2">
              <Field label="Anbindungs-Hinweis (optional)">
                <input
                  className={inputClass}
                  value={form.interfaceNote}
                  onChange={(e) => setForm({ ...form, interfaceNote: e.target.value })}
                  placeholder="z. B. Anbindung an externe Kunststoffschale …"
                />
              </Field>
            </div>
          </>
        ...
        <Field label="Ingenieur">
          ...
          >
            {state.users
              .filter((u) => u.demoRole === "engineering" || u.demoRole === "manager")
              .map((u) => (
        ...
        <Field label="Bezugsentwickler">
          <select
            ...
            onChange={(e) => setForm({ ...form, coverDeveloperUserId: e.target.value })}
          >
            {state.users
